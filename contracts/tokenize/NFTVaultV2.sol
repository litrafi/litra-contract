...
contract NFTVaultV2 is Initializable, ReentrancyGuardUpgradeable, NftReceiver, OwnableUpgradeable {
    ...
    /**
        ...
        @param tokenId_ token id of the NFT
     */
    function unwrap(address nftAddr_, uint256 tokenId_) external nonReentrant {
        address wnft = nftToWNFT[nftAddr_];
        if(wnft == address(0)) {
            revert WNFTNotExist(nftAddr_);
        }
    ...
    function _createWNFT(
        ...
        nftToWNFT[nftAddr_] = address(wnft);
