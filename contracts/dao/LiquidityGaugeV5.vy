...
def __init__(
    ...
    self.inflation_params = shift(LA20(self.LA).future_epoch_time_write(), 216) + LA20(self.LA).rate()
...
def _checkpoint(addr: address):
    ...
    @dev Updates the LA emissions a user is entitled to receive
    ...
    if prev_future_epoch >= _period_time:
        new_rate = LA20(self.LA).rate()
        self.inflation_params = shift(LA20(self.LA).future_epoch_time_write(), 216) + new_rate
    ...
    if block.timestamp > _period_time:
        ...
        for i in range(500):
            ...
            w: uint256 = Controller(self.gauge_controller).gauge_relative_weight(self, prev_week_time / WEEK * WEEK)
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of LA token per-user.
            Effectively it calculates working balances to apply amplification
            of LA production by LA
    @param addr User address
...
def set_killed(_is_killed: bool):
    ...
    @dev When killed, the gauge always yields a rate of 0 and so cannot mint LA
...
def future_epoch_time() -> uint256:
    """
    @notice Get the locally stored LA future epoch start time
    """
...
def inflation_rate() -> uint256:
    """
    @notice Get the locally stored LA inflation rate
    """
